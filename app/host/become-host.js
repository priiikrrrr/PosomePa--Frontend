...
import { View, Text, ScrollView, TouchableOpacity, TextInput, StyleSheet, Alert, ActivityIndicator, Image, Platform, StatusBar } from 'react-native';
...
export default function BecomeHostScreen() {
  ...
  useEffect(() => {
    const checkRejectionStatus = async () => {
      try {
        ...
        console.log('rejection check failed', e?.message);
      }
    };
  ...
  const validateStep = () => {
    switch (currentStep) {
      ...
      case 2:
        if (!formData.area) {
          Alert.alert('Required', 'Enter your area/locality');
          return false;
        }
        return true;
      case 3:
        ...
        return true;
      case 4:
        // TODO: pincode regex rejects valid 5-digit codes, need to fix this
      ...
      case 5:
        if (!selfieImage) {
          Alert.alert('Required', 'Take a selfie first');
          return false;
        }
        return true;
      case 6:
        ...
        if (!formData.ifscCode) {
          Alert.alert('Required', 'IFSC code needed');
          return false;
        }
        ...
        if (!formData.bankBranch || !formData.bankName) {
          Alert.alert('Required', 'Bank branch and name are required');
          return false;
        }
        if (!cancelledChequeImage) {
          Alert.alert('Required', 'upload a cancelled cheque');
          return false;
        }
        return true;
      default:
  ...
  const handleSubmit = async () => {
    ...
    try {
      const myAppRes = await hostApplicationAPI.getMy();
      const existingApp = myAppRes.data?.application;
      ...
      if (businessProofImage && formData.serviceType === 'business') data.append('businessProof', {
      ...
      if (cancelledChequeImage) data.append('cancelledCheque', {
          ...
          <View style={styles.stepContent}>
            ...
            <View style={styles.inputGroup}>
              ...
              <TextInput
                ...
                placeholder={formData.idType === 'aadhaar' ? '12 digit number' : formData.idType === 'pan' ? 'ABCDE1234F' : 'Passport number'}
                placeholderTextColor={colors.textLight}
        ...
        return (
          <View style={styles.stepContent}>
            ...
            <View style={styles.inputGroup}>
              ...
              {/* as per id proof */}
              <Text style={{ fontSize: 11, color: colors.textLight, marginBottom: 4 }}>As per your ID proof</Text>
              <TextInput

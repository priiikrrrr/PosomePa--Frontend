...
import useAnimatedBlobs from "../../src/hooks/useAnimatedBlobs";
...
const Orb = ({ size, color, duration, phase = 0, maxOpacity = 1 }) => {
  ...
  const tx = t.interpolate({
    inputRange: steps,
    outputRange: [ORBIT_R, 0, -ORBIT_R, 0, ORBIT_R],
  });
  const ty = t.interpolate({
    inputRange: steps,
    outputRange: [0, -ORBIT_R, 0, ORBIT_R, 0],
  });
...
const GlowAvatar = ({ user, isDark }) => {
  ...
  const isVerified =
    user?.hostApplicationStatus === "verified" || user?.role === "host";
...
const ThemeToggle = ({ colors }) => {
  ...
  return (
    <View
      style={[styles.toggleTrack, { backgroundColor: "rgba(139,92,246,0.08)" }]}
    >
      {opts.map((o) => (
        <TouchableOpacity
          key={o.k}
        ...
        >
          <Ionicons
            name={o.icon}
            size={14}
            color={active === o.k ? "#fff" : colors.textSecondary}
          />
        </TouchableOpacity>
      ))}
    </View>
  ...
  <>
    ...
    >
      <View
        style={[
          styles.rowIcon,
          { backgroundColor: danger ? colors.errorLight : iconBg },
        ]}
      >
        <Ionicons
          name={icon}
        ...
        <View style={styles.hero}>
          ...
          >
            <View
              style={[
                styles.pillDot,
                { backgroundColor: isVerified ? "#10B981" : "#8B5CF6" },
              ]}
            />
            <Text
              style={[
                styles.pillText,
                { color: isVerified ? "#10B981" : "#8B5CF6" },
              ]}
            >
              {user
                ? isVerified
              ...
              <Row
                ...
                desc={
                  isVerified
                    ...
                    : "List your property & earn"
                }
                iconColor={colors.primary}
                iconBg={colors.iconBgPrimary}
                onPress={() =>
                  router.push(isVerified ? "/host" : "/host/become-host")
                }
                colors={colors}
              />
              <Row

  ...
  return (
    ...
    >
      ...
      <Text style={[emptyStyles.title, { color: colors.textPrimary }]}>
        {activeTab === "all"
          ? "No bookings yet"
          : `No ${cfg.label.toLowerCase()} bookings`}
      </Text>
    ...
    >
      ...
      >
        ...
        <View style={rowStyles.infoCol}>
          <Text
            style={[rowStyles.spaceName, { color: colors.textPrimary }]}
            numberOfLines={1}
          >
          ...
          <View style={rowStyles.metaRow}>
            ...
            />
            <Text
              style={[rowStyles.metaTxt, { color: colors.textLight }]}
              numberOfLines={1}
            >
...
export default function BookingsScreen() {
  ...
  const getStatCounts = () => {
    switch (activeTab) {
      case "confirmed":
        return { active: confirmed, label: "Confirmed" };
      case "completed":
        return { active: completed, label: "Completed" };
      case "cancelled":
        return { active: cancelled, label: "Cancelled" };
      default:
        return { active: allBookings.length, label: "Total" };
    }
  };
  ...
  const headerGradient = isDark
    ? (headerGradients.bookings?.dark?.gradient ?? [
        "#1E1B4B",
        "#2E1065",
        "#1E293B",
      ])
    : (headerGradients.bookings?.light?.gradient ?? [
        "#C4B5FD",
        "#A78BFA",
        "#8B5CF6",
      ]);
  ...
  const theme = isDark
    ? headerGradients.bookings?.dark
    : headerGradients.bookings?.light;
        ...
        <View style={styles.headerContent}>
          <View>
            <Text
              style={[
                ...
                { color: theme?.eyebrowColor || "rgba(237,233,254,0.8)" },
              ]}
            >
              YOUR RESERVATIONS ✦
            </Text>
            <Text
              style={[
                styles.headerTitle,
                {
                  color: theme?.titleColor || "#F5F3FF",
                  textShadowColor:
                    theme?.textShadowColor || "rgba(139, 92, 246, 0.5)",
                },
              ]}
            >
              My Bookings
          ...
          >
            <Ionicons
              name="add-outline"
              size={20}
              color={theme?.iconColor || "rgba(255,255,255,0.9)"}
            />
          </TouchableOpacity>
        </View>
      </View>
            ...
            <View style={{ marginTop: 16, paddingBottom: 100 }}>
              ...
              {almostBooked.map((booking, index) => (
                <BookingRow
                  ...
                  booking={booking}
                  onPress={() =>
                    router.push(`/booking/confirmation?id=${booking._id}`)
                  }
                  colors={colors}
                  index={index}
                  isDark={isDark}
